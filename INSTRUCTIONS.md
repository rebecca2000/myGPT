...
Build a lookup-table model (`nn.Embedding(vocab, vocab)`) with no attention.
Purpose: get the training loop, cross-entropy loss, and sampling working before
attention exists, so later milestones have a baseline to beat.

**Success criteria**
- [ ] Data loads and encodes/decodes losslessly (round-trip test passes).
- [ ] Training loop runs and training loss **decreases** over iterations.
- [ ] Model can sample text of arbitrary length without crashing.
- [ ] Final validation loss is recorded (baseline number to beat later).
